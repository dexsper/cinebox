...
- На постере видно, если уже смотрели
...
5. **Трейлеры.** С TMDB приходит ключ ролика, Cinebox достает подписанные ссылки YouTube и открывает их в том же mpv.
6. **Сеть.** TMDB и парсер могут идти через системный прокси. Если этот путь не проходит и включен обход DNS, хост резолвится по DoH и запрос уходит напрямую. TorrServer без прокси.
